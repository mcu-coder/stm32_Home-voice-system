...
int8_t gizwitsEventProcess(eventInfo_t *info, uint8_t *gizdata, uint32_t len)
{
    ...
    for(i=0; i<info->num; i++)
    {
        switch(info->event[i])
        {
        case EVENT_stepperMotor:
            currentDataPoint.valuestepperMotor = dataPointPtr->valuestepperMotor;
            GIZWITS_LOG("Evt: EVENT_stepperMotor %d \n", currentDataPoint.valuestepperMotor);
        ...
        case EVENT_LED:
			...
			ledluminance.ledDutyRatio = currentDataPoint.valueLED;
			currentDataPoint.valueLED = ledluminance.ledDutyRatio;	//LED亮度上传云平台
			TIM_SetCompare2(TIM1, ledluminance.ledDutyRatio);	//显示亮度	
        ...
        case EVENT_fan:
			...
			if (!systemState.mode)
			{
				fanluminance.FanDutyRatio = currentDataPoint.valuefan;
				currentDataPoint.valuefan = fanluminance.FanDutyRatio;	//风速数据上传至云平台
				TIM_SetCompare3(TIM1, fanluminance.FanDutyRatio);	//显示风速	
			}
...
void userHandle(void)
{
    ...
    currentDataPoint.valuesmokeAlarm = RESET;//Add Sensor Data Collection
    currentDataPoint.valuetemperature = sensorData.temp;//Add Sensor Data Collection
    currentDataPoint.valuehumidness = sensorData.humi;//Add Sensor Data Collection
    currentDataPoint.valuelux = sensorData.lux;//Add Sensor Data Collection
    currentDataPoint.valuesmoke = sensorData.smoke;//Add Sensor Data Collection
...
void userInit(void)
{
    ...
    currentDataPoint.valuestepperMotor = ;
    currentDataPoint.valuehumidifier = ;
    currentDataPoint.valuesmokeAlarm = ;
    currentDataPoint.valuemodel = ;
    currentDataPoint.valueLED = ;
    currentDataPoint.valuetemperature = ;
    currentDataPoint.valuehumidness = ;
    currentDataPoint.valuefan = ;
    currentDataPoint.valuelux = ;
