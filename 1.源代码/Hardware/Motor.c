...
static void MOTOR_Rhythm_4_1_4(uint8_t step,uint8_t dly)
{
	switch(step)
	{
		case 0 :
...
static void MOTOR_Rhythm_4_2_4(uint8_t step,uint8_t dly)
{
	switch(step)
	{
		case 0 :
...
static void MOTOR_Rhythm_4_1_8(uint8_t step,uint8_t dly)
{
	switch(step)
	{
		case 0 :
...
static void MOTOR_Direction(uint8_t dir,uint8_t num,uint8_t dly)	//方向 节拍 频率
{
	if(dir)
	{
		switch(num)
...
static void MOTOR_Direction_Angle(uint8_t dir,uint8_t num,u16 angle,uint8_t dly) //方向 节拍  角度 频率
{
...
{
	if (systemState.motorCommand.motorAnterogradeFlag)
	{
		systemState.motorCommand.motorLocation = motorLocation_ON;
		MOTOR_Direction_Angle(1, 0, 90, 1);
		systemState.motorCommand.motorAnterogradeFlag = 0;
		MOTOR_STOP();
	}
	if (systemState.motorCommand.motorReversalFlag)
	{
		systemState.motorCommand.motorLocation = motorLocation_OFF;
		MOTOR_Direction_Angle(0, 0, 90, 1);
		systemState.motorCommand.motorReversalFlag = 0;	
		MOTOR_STOP();
	}
}
